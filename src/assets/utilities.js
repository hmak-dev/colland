...
export {
  Lab_to_LCH, LCH_to_Lab, OKLab_to_OKLCH, OKLCH_to_OKLab,
} from './conversions.js';
...
export function HWB_to_sRGB(HWB) {
  ...
  const rgb = HSL_to_sRGB([hue, 1, 0.5]);
...
export function naive_CMYK_to_sRGB(CMYK) {
  ...
  const cyan = CMYK[0]; const magenta = CMYK[1]; const yellow = CMYK[2]; const
    black = CMYK[3];
...
export function naive_sRGB_to_CMYK(RGB) {
  ...
  const red = RGB[0]; const green = RGB[1]; const
    blue = RGB[2];
